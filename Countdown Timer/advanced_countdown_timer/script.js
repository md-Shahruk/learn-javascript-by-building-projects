...
class CountdownTimer{
    ...
    // Update Time Display
    updateTimeDisplay(){
        const totalSeconds = this.isClockRunning ? this.remainingTime : this.getTotalSeconds();
    ...
    updateProgress(){
        ...
        this.clockCircle.style.background =
        `conic-gradient(var(--primary) ${progress}%, transparent ${progress}%)`;
    ...
    start(){
        ...
        [this.inputHours, this.inputMinutes, this.inputSeconds].forEach((input) => this.validInput(input));
